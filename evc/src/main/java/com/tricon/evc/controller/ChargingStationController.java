...
//  "http://172.16.17.30:8066/stations?startingLatitude="+15.2345+"&startingLongitude="+23.3432+"&radius=5&chargerType=1timeStamp=2008-09-15T15:53:00+05:00"
    ...
    @Autowired
    ChargingStationServiceImpl chargingStationServiceObject;
    ...
    public CustomResponse getStations(
            @RequestParam(value = "startingLatitude") double startingLatitude,
            ...
            @RequestParam(value = "timeRequired", defaultValue = "99") int timeRequired

    ) {

        LOGGER.info("Entering into the getStations Class in the Controller");

        return chargingStationServiceObject.getStations(startingLatitude, startingLongitude, endingLatitude,
                endingLongitude, radius, chargerType, companyId, timeStamp, timeRequired);

    }


    @RequestMapping(value = "/stations/{id}", method = RequestMethod.GET)
    public CustomResponse getStationDetails(@PathVariable String id) {
